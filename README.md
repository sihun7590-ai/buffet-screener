...
- **최대 낙폭은 분기말 14개 시점만 찍어서 잰 값이라 실제보다 훨씬 작게 나옵니다.** 재보면 같은 기간 SPY가 분기말 기준 -8.0%인데 일별 종가로는 -19.0%(2025-04-08 저점), **2.4배** 차이입니다. 분기 안에서 떨어졌다가 분기말 전에 회복한 구간은 분기말 표본에 아예 안 잡히는데, 실제로 들고 있었다면 겪었을 건 일별 쪽입니다. 제대로 고치려면 모든 보유 종목의 일별 가격이 필요하고(분기 스냅샷이 아니라 500종목 일별 히스토리), 그전까지는 표 라벨·툴팁 모두에 "분기말 기준"이라고 명시하는 게 정직한 처리입니다.
...
종목 상세 페이지에 그 회사 임원·이사·10% 이상 보유주주의 최근 자사주 매매 내역을 보여줍니다 ([lib/insiderTrading.ts](lib/insiderTrading.ts), [components/InsiderActivity.tsx](components/InsiderActivity.tsx)). 미국 증권법상 이들은 자기 회사 주식을 사거나 팔면 영업일 기준 2일 이내에 SEC에 Form 4로 신고해야 하는데, 그 신고 내역을 그대로 보여주는 것입니다.

- **출처**: 재무제표와 같은 SEC EDGAR — 여기서는 두 단계를 거칩니다. 먼저 [submissions API](https://data.sec.gov/submissions/)로 그 회사가 최근 제출한 Form 4 목록(제출일·접수번호)을 받고, 그중 최근 5건에 대해 실제 Form 4 XML 문서를 각각 내려받아 파싱합니다. CIK 조회는 [lib/xbrl.ts](lib/xbrl.ts)의 `loadTickerCikMap()`을 그대로 재사용합니다 — 티커→CIK 매핑에는 이미 예외 처리(CIK_OVERRIDES)가 있어서, 따로 하나 더 만들면 둘이 서로 다르게 어긋날 위험만 생깁니다.
- **회사 소개·최근 뉴스와 같은 이유로 배치 갱신 대상이 아닙니다.** 이런 신고는 언제 올라올지 예측할 수 없고, 오래된 매매 내역은 의미가 크게 줄어듭니다. 그래서 `data/scores.json`에 굽지 않고 페이지를 열 때마다 실시간으로 가져오며, 실패해도(신고 없음, 네트워크 오류, XML 파싱 실패 등) 빈 배열을 돌려주고 나머지 페이지는 그대로 뜹니다.
- **submissions API의 `primaryDocument` 필드를 그대로 쓰면 안 됩니다.** 이 필드는 대개 `xslF345X06/form4.xml` 같은 경로인데, 이름과 달리 **SEC가 XSLT로 미리 렌더링한 사람이 읽기 좋은 HTML 페이지**를 가리킵니다 — 정작 우리가 파싱해야 할 원본 XML은 같은 접수 폴더 바로 밑에 자기 파일명(`form4.xml`, `doc4.xml` 등 제출자마다 다름)으로 따로 있습니다. 그래서 경로의 마지막 세그먼트만 잘라서 원본 XML 파일명으로 씁니다.
- **submissions 목록에는 "이 회사 주식에 대한 신고"와 "이 회사가 남의 주식을 사고팔았다는 신고"가 섞여 들어옵니다.** Form 4는 발행사(issuer)와 신고자(reporting owner) 양쪽 CIK로 색인되기 때문입니다. 엑슨모빌은 ProPetro(PUMP)의 10% 이상 주주라, 엑슨 목록에 ProPetro 주식 매매 신고가 딸려 옵니다 — 걸러내지 않으면 **엑슨 페이지에 남의 회사 주식이, 남의 회사 주가($16.66 vs 엑슨의 약 $155)로 표시됩니다.** 버크셔도 같은 이유로 최근 Form 4 상당수가 남의 회사 것입니다. 그래서 XML을 파싱한 뒤 `issuer.issuerCik`이 조회 대상 회사와 일치하는 신고만 남깁니다. 발행사가 누구인지는 문서를 받아 봐야 알 수 있어서 후보를 12건까지 훑되 자사 신고 5건을 채우면 즉시 멈춥니다 — 대부분의 회사는 첫 5건이 전부 자사 것이라 추가 요청이 아예 없고, 버크셔·엑슨 같은 대형 보유자에서만 더 들어갑니다.
- **새 의존성: [fast-xml-parser](https://github.com/NaturalIntelligence/fast-xml-parser).** Form 4는 실제 법정 공시 XML이라 속성 순서·공백·self-closing 태그가 제출자마다 제각각이고, 여기 나오는 숫자는 실제 매매 주식수·단가입니다. 정규식으로 뽑아내는 건 이런 데이터에 감당하기 힘든 정확성 리스크라고 판단해, 의존성 6개를 최소한으로 유지해 온 이 프로젝트에 예외적으로 하나를 더 추가했습니다 (의존성 없는 라이브러리, MIT 라이선스, 매우 작음).
- **fast-xml-parser의 전형적인 함정 하나를 그대로 맞았습니다.** 한 Form 4가 매매를 하나만 담으면 `nonDerivativeTransaction`이 객체로 파싱되고, 여러 건을 담으면(같은 신고 한 건에 여러 매매가 묶인 경우가 실제로 흔합니다) 배열로 파싱됩니다. 파싱 직후 무조건 배열로 정규화하는 헬퍼를 거치지 않으면 다건 신고에서 매매가 조용히 누락됩니다.
- **범위 제한: `nonDerivativeTable`만 다루고 `derivativeTable`(스톡옵션 부여, RSU 베스팅과 그 행사)은 이번 버전에서 뺐습니다.** 파생 거래는 예정된 보상 이벤트로 발생하는 경우가 대부분이라 노이즈가 크고, 사람들이 이 표에서 실제로 보고 싶어하는 "본인 의사로 사고판 거래"와 섞으면 그 신호가 묻힙니다.
- **매수와 매도는 같은 무게로 읽으면 안 됩니다.** 매수는 자기 돈을 들이는 것이니 긍정적으로 읽히는 경우가 많지만, 매도는 세금·자산 분산·주택 구입·미리 짜둔 10b5-1 매매 계획 등 회사 전망과 무관한 이유로도 얼마든지 일어납니다. 화면의 안내 문구(`messages/*.json`의 `insiderTrading.caveat`)에도 이 비대칭을 그대로 적어 뒀습니다 — 매도를 매수의 반대 신호로 자동으로 읽지 말라는 뜻입니다.
