...
- [ ] **T15. 사이드바 하단에 남은 죽은 컨트롤 2개** — `components/Sidebar.tsx:59-76`
  - **"가중치 설정"** (`:60`) — `href="/"`. 가중치 패널을 여는 것처럼 보이지만 대시보드로 이동만 함.
    실제 패널은 대시보드 우측 상단 "가중치 조정" 버튼이 엶
  - **"다크 모드"** (`:70`) — `<button>`이 아니라 `<div>`. 핸들러 없음, 토글 모양만 그려진 장식.
    리디자인(`48654dd`)에서 `ThemeToggle.tsx`가 삭제된 흔적 (T6과 같은 뿌리)
  - 2026-08-27에 같은 이유("아무 데도 못 감")로 **종목상세·계정 항목은 제거함**. 이 둘은 사용자가 지목하지 않아 보류
  - 선택지: (a) 둘 다 제거 (b) 가중치는 대시보드 패널을 여는 실제 동작 부여, 다크 모드는 제거
