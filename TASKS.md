...
## 💳 유료화 (2026-09-17 착수 — 계획과 막힌 지점은 [ROADMAP.md](ROADMAP.md) 맨 위)

- [x] **PAY-1. 등급 계층 (Free / Pro / Premium) — 결제 없이, 기본 전체 개방**
  - 규칙은 **`lib/entitlements.ts`의 `FEATURES` 한 곳**. 기능을 다른 등급으로 옮기려면 여기서 한 단어만 바꿈
  - 스위치: 서버 환경변수 `PAYWALL_ENABLED=true`. **없으면 모두 premium 취급 → 현재 사이트 그대로**
    (결제 수단 없이 잠기면 기존 사용자가 쓰던 기능을 돈 낼 방법도 없이 잃으므로, 설정 누락 시 "열림"이 맞는 실패 방향)
  - 등급 결정: `lib/supabase/entitlement.ts` `getViewerTier()` — 비로그인·행 없음·만료·알 수 없는 값·DB 오류 **전부 free**. React `cache`로 요청당 1회 조회
  - DB: `supabase/migrations/005_user_entitlements.sql` — **아직 실행 불필요** (페이월 꺼져 있으면 조회 안 함).
    **쓰기 정책이 없음** = 브라우저(anon key)로는 등급 변경 불가, service_role(향후 결제 웹훅)만 씀. 다른 테이블과 다르게 의도적
  - 개발용: `PAYWALL_PREVIEW_TIER=free|pro|premium` — **프로덕션 빌드에선 무시** (Vercel에 잘못 넣어도 유료 등급이 풀리지 않게)
  - 현재 배분: Free 2개(스크리너·관심종목) / Pro +12개 / Premium +3개(백테스트·조건 검색·CSV)
    - 관심종목은 스펙상 Pro였지만 **Free로 둠** — 가입할 이유 자체라서. 그 위의 알림·감시 기능은 Pro
  - **게이트는 서버에서 겁니다.** 잠긴 패널은 렌더도, 데이터 fetch도 안 함 (Form 4·점수 이력 호출 생략)
  - **대시보드는 데이터를 잘라서 보냄** (`redactScore`) — 클라이언트 컴포넌트라 props가 전부 페이로드에 실리기 때문.
    free에는 채점 항목·내재가치·안전마진이 **아예 전달되지 않음** (페이로드 1,703KB → 326KB)
  - 잠금 카드에 **구매 버튼 없음** — 결제가 없는데 버튼을 두면 또 하나의 "눌러도 아무 일 없는 컨트롤". 결제 연동 시 `components/LockedFeature.tsx` 한 곳에 추가
  - 알고 남긴 것: `isBuyCandidate` 배지는 free에도 보임 → 약 27개 종목의 안전마진 **부호**(크기 아님)를 추론 가능. 무료 등급의 핵심 출력이라 유지
  - ⚠️ 결합 주의: 포트폴리오 행에 안전마진이 들어 있음. `fairValue`를 `portfolio`보다 높은 등급으로 옮기면 포트폴리오도 잘라서 보내야 함 (`entitlements.ts` 주석)

  **검증** (Supabase를 빈 값으로 띄운 로컬 **프로덕션 빌드**, `PAYWALL_ENABLED=true` + 비로그인 = 실제 free 경로):
  | | 결과 |
  |---|---|
  | 규칙 (실데이터 498종목) | 상위 등급이 기능을 잃는 경우 0 · 조작된 등급값(`enterprise`, `PRO`, 빈값) 전부 거부 · free에서 채점 항목·내재가치·안전마진 전 종목 제거 · pro/premium은 원본 그대로 · 원본 스냅샷 변형 없음 |
  | 대시보드 (free) | 툴바 3개 잠금(가중치 Pro · 조건검색/CSV Premium), CSV 버튼 없음, 정렬에서 안전마진 제외, 카드 안전마진 표시 0개, **페이지 페이로드에 채점값·안전마진 수치 없음** |
  | 종목 상세 (free) | 7개 패널 잠금 + 상단 내재가치·안전마진 잠금, 점수 막대·뉴스·차트는 표시. **AAPL의 실제 숨김 값 10개(원값·포맷값) HTML 전체에서 0건** |
  | 백테스트 (free) | 제목 + 잠금만, KPI·표 없음 |
  | **페이월 꺼짐** | 잠금 0개, 버튼 3개·안전마진 정렬·카드 60개 안전마진·종목 전 패널·백테스트 표 → **기존과 동일** |
  | ⚠️ 못 본 것 | **MY Page** (로그인 필요 — Supabase를 끄면 비로그인이라 로그인 안내만 나옴). 코드 경로는 대시보드·종목과 같은 `can()` 패턴 |

- [ ] **PAY-2. 시세 데이터 후보 비교표** — Yahoo 대체 (라이선스 조건 · 월 비용 · 필요한 엔드포인트: 일봉 히스토리, 현재가, 가능하면 뉴스). **다음 작업**
- [ ] **PAY-3. 사업자등록 · 통신판매업 · 유사투자자문업 신고** — 사용자 진행 (코드 작업 아님)
- [ ] **PAY-4. 시세 소스 교체** — `lib/price.ts`의 `fetchPriceHistory` 한 곳이 입구라 교체 지점은 좁음. `lib/news.ts` · `lib/tradingview.ts`도 Yahoo
- [ ] **PAY-5. 결제 연동** (토스페이먼츠/포트원) → 웹훅이 service_role로 `user_entitlements` 기록 → 잠금 카드에 구매 버튼
- [ ] **PAY-6. 오픈** — Vercel Pro 전환 → 마이그레이션 005 실행 → `PAYWALL_ENABLED=true`

---

## 📱 모바일 대응 (2026-08-27)

- [x] **M1. 사이드바가 모바일에서 화면을 잡아먹던 문제** — `components/Sidebar.tsx:26`
  - 원인: `w-[236px] shrink-0`에 **반응형 접두사가 전혀 없었음.** 375px 화면의 63%를 차지하고 본문에 ~139px만 남김
  - 조치: `hidden lg:flex` (1024px 미만에서 숨김) + 내비 3개를 헤더 아래 가로줄로 이동 (`components/MobileNav.tsx`)
  - 햄버거 드로어를 만들지 않은 이유: 링크가 3개뿐이라 320px 줄에도 여유롭게 들어감.
    드로어는 탭·패널·오버레이·포커스 관리를 추가하면서 애초에 방해되지도 않던 링크 3개를 숨기는 일
  - 사이드바 안에만 있던 "데이터 기준일" 카드는 모바일에서 사라짐 — 종목 페이지가 같은 정보를 본문에 갖고 있음

- [x] **M2. 헤더가 375px에서 넘침** — `components/SiteHeader.tsx`, `components/HeaderSearch.tsx`
  - 검색창 `min-w-[200px]` → `min-w-0`. flex 아이템의 기본 `min-width:auto`가 축소를 거부해 계정 컨트롤을 화면 밖으로 밀어냄
  - 사이드바가 숨으면 로고도 사라지므로 **헤더에 모바일 전용 로고 마크** 추가 (워드마크는 검색창 폭을 뺏어서 제외)
  - 장식용 알림 벨은 모바일에서 숨김 (`sm:grid`) — 뒤에 알림 시스템이 없는 아이콘
  - "MY Page" 라벨은 모바일에서 숨기고 아바타만 (같은 목적지가 바로 아래 내비줄에 있음)
  - 검색 드롭다운에 `min-w-[240px] sm:min-w-0` — 검색창이 110px로 줄면 드롭다운이 그 폭을 물려받아 회사명이 잘렸음

- [x] **M3. 백테스트 KPI가 고정 4열** — `app/[locale]/backtest/page.tsx:70`
  - `repeat(4, minmax(0,1fr))` → `grid-cols-2 lg:grid-cols-4`. 모바일에서 KPI 하나가 ~80px로 눌려 26px 숫자가 3줄로 깨졌음

### 검증한 것 / 못 한 것

| | |
|---|---|
| ✅ 브라우저 실측 (375 · 820 · 1280px) | 가로 오버플로 **0px**, 뷰포트 밖 요소 **0개**, 사이드바 전환, 모바일 내비 3링크(307px), MobileNav이 헤더 바로 아래 64px에 정확히 안착(겹침 0), 검색 드롭다운 240px로 확장·화면 안에 안착 |
| ✅ 프로덕션 CSS | `.lg\:flex` `.lg\:hidden` `.lg\:grid-cols-4` 및 40rem/64rem 미디어쿼리 생성 확인 |
| ✅ 정적 감사 | 넓은 테이블 8개 전부 `overflow-x-auto` 안에 있음, 고정 그리드는 M3 하나뿐이었음 |
| ⚠️ **못 봄** | 로그인 게이트 뒤 페이지(스크리너·종목상세·MY Page·백테스트)의 실제 렌더. **레이아웃 셸만 실측했고 본문은 마크업으로 판단** |
