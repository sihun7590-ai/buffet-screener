...
| 날짜 | 작업 | 커밋 |
|---|---|---|
| 2026-09-17 | **유료화 착수 — 등급 계층만 (PAY-1)**. 조사 결과 결제 전에 코드 밖 문제 4개가 먼저: 유사투자자문업 신고, Yahoo 시세의 상업적 이용 금지(모든 점수의 주가가 Yahoo), Stripe 한국 가맹 불가, Vercel Hobby 상업 이용 금지. 사용자 결정: 신고는 아직 없음 · 시세는 후보 비교 후 결정 · 이번엔 권한 계층만. **`PAYWALL_ENABLED` 미설정 시 전체 개방 → 배포해도 사이트 동작 불변** (페이월 on/off 양쪽 프로덕션 빌드로 실측). 상세·검증표는 TASKS PAY-1, 막힌 지점은 ROADMAP 맨 위 | — |
